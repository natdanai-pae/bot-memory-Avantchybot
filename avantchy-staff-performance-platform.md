...
- Use this approach for the next evaluation-summary work by default. The layout can change to fit the available evidence, number of staff, team vs individual scope, and key message, but the evaluation framework and interpretability should remain consistent.
- Priority template for GAA performance evaluation: when the user asks for a GAA evaluation summary, start with this evidence-and-growth dashboard pattern as the preferred template, not a generic infographic or raw report. Adapt layout and density to the source data, but preserve the 5D evaluation logic, radar detail, readiness/trend comparison, and coaching-oriented executive reading.
